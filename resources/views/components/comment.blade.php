<div class="flex gap-4 mb-6" id="comment-{{ $comment->id }}">
    ...
    <div class="flex-1">
        <div class="bg-gray-50 rounded-lg p-4">
            ...
            <div class="text-gray-700 leading-relaxed whitespace-pre-line mb-3">{{ $comment->content }}</div>
            
            <!-- Vote and Reply Actions -->
            <div class="flex items-center gap-4 text-sm">
                @auth
                    <!-- Like Button -->
                    <button class="vote-btn flex items-center gap-1 text-gray-600 hover:text-green-600 transition-colors"
                            data-comment-id="{{ $comment->id }}"
                            data-vote-type="like"
                            @if($comment->getUserVote(auth()->id())?->vote_type === 'like') data-active="true" @endif>
                        <svg class="w-5 h-5 {{ $comment->getUserVote(auth()->id())?->vote_type === 'like' ? 'fill-green-600 text-green-600' : '' }}" 
                    ...
                    <!-- Dislike Button -->
                    <button class="vote-btn flex items-center gap-1 text-gray-600 hover:text-red-600 transition-colors"
                            data-comment-id="{{ $comment->id }}"
                            data-vote-type="dislike"
                            @if($comment->getUserVote(auth()->id())?->vote_type === 'dislike') data-active="true" @endif>
                        <svg class="w-5 h-5 {{ $comment->getUserVote(auth()->id())?->vote_type === 'dislike' ? 'fill-red-600 text-red-600' : '' }}" 
                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Reply Button -->
                @if($comment->canReply())
                    <button class="reply-toggle-btn flex items-center gap-1 text-indigo-600 hover:text-indigo-800 transition-colors font-medium"
                            data-comment-id="{{ $comment->id }}">
        ...
        @if($comment->canReply())
            <div class="reply-form-container mt-4 hidden" id="reply-form-{{ $comment->id }}">
                <form method="POST" action="{{ route('comments.reply', $comment->id) }}" class="bg-gray-100 rounded-lg p-4">
                    @csrf
                    ...
                    <textarea name="content" required rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none text-sm mb-3"
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button" class="cancel-reply-btn px-4 py-2 bg-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-400 transition-colors"
                            data-comment-id="{{ $comment->id }}">
                            Anuluj
                        </button>
                    </div>
